      ...
      {seats.map((seat) => (
        <button
          ...
          className={`relative p-4 rounded-lg flex flex-col items-center justify-center transition-all duration-200 ${
            seat.is_booked
              ? 'cursor-not-allowed' // removed bg-red-100 since we'll use image
              : selectedSeat === seat.id
              ? 'bg-yellow-100'
              : 'bg-green-100 hover:bg-green-200 cursor-pointer'
          }`}
        >
          {/* START CHANGE: Show image if booked, seat number otherwise */}
          {seat.is_booked ? (
            <img
              src="/img.png" // put your booked seat image in public folder
              alt="Booked"
              className="w-23 h-12"
            />
          ) : (
            <span className="text-lg font-medium text-green-800">{seat.seat_number}</span>
          )}
          {/* END CHANGE */}

          {/* Ping indicator for selected seat */}
